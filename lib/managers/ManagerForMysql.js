...
SchemaManagerForMysql.prototype = {
	...
	fetchByParams: function( type, params, callback ) {
		pool.getConnection(function(err, conn) {
			if(err) {
				if(!callback) {
	...
	persist: function( type, object, callback ) {
		pool.getConnection(function(err, conn) {
			...
			if(type[type.table.keys.primary[0]] == null) {
				var column = utils.findColumnByName(type.table.keys.primary[0], type.table.columns);
				if(column && column.type.name == 'Buffer') {
					type[type.table.keys.primary[0]] = new Buffer(utils.generateUUID().replace(/\-/g,''),"HEX");
				}
			}
	...
	purge: function( type, id, callback ) {
		pool.getConnection(function(err, conn) {
			if(err) {
				if(!callback) {
