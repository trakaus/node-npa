	...
	this.pool.getConnection(function(err, conn) {
	    ...
	    type.table.columns.forEach(function(currentValue, index, source) {
			if(currentValue) {
				...
				/*
				values.push((currentValue.type.name == 'Buffer' ? 
					(object[currentValue.name] !== null ? 
						(typeof object[currentValue.name] === 'object' ? 
							object[currentValue.name].toString('hex') : 
							object[currentValue.name].toString()) : 
						null) : 
					object[currentValue.name]));
				*/
			}
		});
